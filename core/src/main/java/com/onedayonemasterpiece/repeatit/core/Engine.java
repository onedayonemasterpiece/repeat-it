...
public final class Engine {
    ...
    /** Daily local-time gate for NEW presentations. It may cross midnight (for example 07:40–00:30). */
    public static final class Window {
        ...
        public Instant next(Instant time) {
            ...
            if(overnight())return d.atTime(start).atZone(zone).toInstant(); // blocked interval is [end,start)
        ...
        public long available(Instant from, Instant to) {
            ...
            long total=0;LocalDate first=from.atZone(zone).toLocalDate().minusDays(1),last=to.atZone(zone).toLocalDate();
        ...
        public Instant add(Instant from, long activeMillis) {
            ...
            while(true){
                ...
                left-=room;cursor=next(close(cursor));if(left==0)return cursor;
            }
        }
    }
    ...
    public static final class Decision {
        ...
        public Instant due;
        /** Remaining successful «Помню» responses; expired is the subset whose requested deadline has passed. */
    ...
    public static Decision next(List<Card> cards,List<Plan> plans,Map<String,State> states,
        ...
        Decision result=new Decision();Map<String,Plan> byDeck=new HashMap<>();
        for(Plan p:plans)if(p.active)byDeck.put(p.deck,p);
        List<Item> dated=new ArrayList<>(),overdue=new ArrayList<>(),undated=new ArrayList<>();
        for(Card c:cards){
            if(!c.active)continue;Plan p=byDeck.get(c.deck);if(p==null){result.withoutPlan++;continue;}
            State s=states.getOrDefault(c.learningKey(),new State());int n=Math.max(0,p.minimum-s.contacts);if(n==0)continue;
            result.remaining+=n;Item x=new Item(c,p,s,n,w.next(s.eligible==null||s.eligible.isBefore(now)?now:s.eligible));
            if(p.deadline==null)undated.add(x);
            else if(!p.deadline.isAfter(now)){result.expired+=n;overdue.add(x);}
            else dated.add(x);
        }
        dated.sort(Comparator.comparing((Item x)->x.p.deadline).thenComparing(x->x.c.key()));
        overdue.sort(Comparator.comparing((Item x)->x.p.deadline).thenComparing(x->x.eligible).thenComparing(x->x.c.key()));
        ...
        for(int i=0;i<dated.size();){
            ...
            l.requiredPerAllowedHour=l.availableMillis==0?null:count*3600000.0/l.availableMillis;result.loads.add(l);
        ...
        if(responseLatencies.size()>=5){
            List<Long> sorted=new ArrayList<>(responseLatencies);Collections.sort(sorted);long median=sorted.get(sorted.size()/2);
        ...
        Item selected=null;Instant bestDue=null;Instant deadlineBase=dated.isEmpty()?null:w.add(now,result.spacingMillis);
        for(Item x:dated){
            ...
            if(selected==null||due.isBefore(bestDue)||(due.equals(bestDue)&&priority(x,selected)<0)){selected=x;bestDue=due;}
        }
        for(Item x:overdue){Instant due=x.eligible;if(selected==null||due.isBefore(bestDue)||(due.equals(bestDue)&&priority(x,selected)<0)){selected=x;bestDue=due;}}
        for(Item x:undated){Instant due=x.eligible;if(selected==null||due.isBefore(bestDue)||(due.equals(bestDue)&&priority(x,selected)<0)){selected=x;bestDue=due;}}
        if(selected!=null){result.cardKey=selected.c.key();result.due=bestDue;}return result;
    }
    private static int priority(Item a,Item b){
        if(a.p.deadline==null&&b.p.deadline!=null)return 1;if(a.p.deadline!=null&&b.p.deadline==null)return -1;
        ...
        d=Integer.compare(b.s.weakDebt,a.s.weakDebt);if(d!=0)return d;return a.c.key().compareTo(b.c.key());
    }

    /** Deadline edits affect future eligibility only; a passed deadline degrades to ordinary spaced learning instead of deleting work. */
    public static State retarget(State original,Plan p,Window w){
        State s=original.copy();if(s.last==null)return s;if(s.contacts>=p.minimum){s.eligible=null;return s;}
        if(p.deadline==null||!p.deadline.isAfter(s.last)){s.eligible=undatedEligible(s.last,s.contacts,s.lastReaction,s.weakDebt,w);return s;}
        int left=Math.max(0,p.minimum-s.contacts);double fraction=s.lastReaction.equals("repeat")?0.25:0.55;
        long gap=Math.max(1,(long)(w.available(s.last,p.deadline)/(double)(Math.max(1,left)+1)*fraction));s.eligible=w.add(s.last,gap);return s;
    }

    /** Credit only a fresh spaced «Помню». A delayed response remains valid even if the calendar deadline has since passed. */
    public static State answer(State old,Plan p,String reaction,Instant shown,Instant answered,Window w){
        ...
        if(answered.isBefore(shown))throw new IllegalArgumentException("clock moved backwards");State s=old.copy();
        if(old.eligible!=null&&shown.isBefore(old.eligible))return s;
        if(reaction.equals("remember")){s.contacts++;s.weakDebt=Math.max(0,s.weakDebt-1);}else s.weakDebt=Math.min(1000,s.weakDebt+1);
        ...
        if(p.deadline==null||!p.deadline.isAfter(answered)){s.eligible=undatedEligible(answered,s.contacts,reaction,s.weakDebt,w);return s;}
        int left=Math.max(0,p.minimum-s.contacts);long horizon=w.available(answered,p.deadline);double fraction=reaction.equals("repeat")?0.25:0.55;
        long gap=Math.max(1,(long)(horizon/(double)(Math.max(1,left)+1)*fraction));s.eligible=w.add(answered,gap);return s;
    }

    private static Instant undatedEligible(Instant from,int successes,String reaction,int weakDebt,Window w){
        Duration gap;if(reaction.equals("repeat")){long minutes=Math.max(15,60/Math.max(1,Math.min(4,weakDebt)));gap=Duration.ofMinutes(minutes);}
        else gap=switch(successes){case 0->Duration.ZERO;case 1->Duration.ofHours(4);case 2->Duration.ofDays(1);case 3->Duration.ofDays(3);default->Duration.ofDays(7);};
