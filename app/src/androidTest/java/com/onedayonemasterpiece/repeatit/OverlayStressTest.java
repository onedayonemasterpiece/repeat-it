...
public final class OverlayStressTest {
    ...
    @Test public void nativeOverlaySurvivesFiniteDelayedFortyCycleJourney() throws Exception {
        ...
        UiObject2 sync=device.wait(Until.findObject(By.text("Обновить карточки сейчас")),UI_TIMEOUT);assertNotNull("Primary refresh control must be visible without scrolling",sync);assertTrue("Refresh control must sit in the primary viewport",sync.getVisibleBounds().bottom<device.getDisplayHeight()*.82);
        UiObject2 pause=device.wait(Until.findObject(By.text("Поставить на паузу")),UI_TIMEOUT);assertNotNull("Stateful pause control must be visible without scrolling",pause);assertTrue("Pause control must sit in the primary viewport",pause.getVisibleBounds().bottom<device.getDisplayHeight()*.9);
        pause.click();UiObject2 resume=device.wait(Until.findObject(By.textContains("Продолжить показы")),UI_TIMEOUT);assertNotNull("Pause tap must visibly change the control",resume);assertEquals("true",store.value("paused","false"));resume.click();UiObject2 pauseAgain=device.wait(Until.findObject(By.text("Поставить на паузу")),UI_TIMEOUT);assertNotNull("Resume tap must visibly restore the control",pauseAgain);assertEquals("false",store.value("paused","false"));
