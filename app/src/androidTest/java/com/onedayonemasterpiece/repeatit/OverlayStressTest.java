...
public final class OverlayStressTest {
    ...
    @Test public void nativeOverlaySurvivesFortyAcceleratedCycles() throws Exception {
        ...
        // Wide test-only window avoids wall-clock flakiness. Production settings remain 07:40–23:00
        // and are covered by the core boundary suite. This state exists only inside the disposable emulator.
        ...
        // Start the FGS while Repeat It owns a foreground activity, then move another app underneath it.
        Intent launch=new Intent(c,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        c.startActivity(launch);Thread.sleep(700);Delivery.start(c);device.pressHome();Thread.sleep(500);
        device.executeShellCommand("am start -a android.settings.SETTINGS");Thread.sleep(600);

        int screenCycles=0,appSwitches=1,rotations=0;
        for(int i=0;i<CYCLES;i++){
            store.forceDue();changed(c);
            UiObject2 remember=waitButton(device,"Помню");
            waitButton(device,"Повторить");
            assertGeometry(store);

            if(i==10||i==29){
                Store.Pending before=store.pending();assertNotNull(before);String pendingId=before.id;
            ...
            if(i>0&&i%5==0){
                ...
                Thread.sleep(450);changed(c);waitButton(device,"Помню");appSwitches++;
            }
