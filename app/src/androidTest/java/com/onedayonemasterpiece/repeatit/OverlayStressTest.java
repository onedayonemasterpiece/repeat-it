...
public final class OverlayStressTest {
    ...
    private static void waitPresentation(Store store) throws Exception {long end=System.currentTimeMillis()+UI_TIMEOUT;while(System.currentTimeMillis()<end){Store.Pending p=store.pending();if(p!=null&&p.shown>0&&!store.value("overlay_geometry","").isEmpty())return;Thread.sleep(60);}fail("Pending was not presented");}
    ...
    @Test public void nativeOverlaySurvivesFiniteDelayedFortyCycleJourney() throws Exception {
        ...
        // Crucially do NOT widen production time settings. agent_debug must work 24h by contract.
        store.put("enabled","true");store.put("paused","false");store.put("sound","false");List<Engine.Card> cards=new ArrayList<>();
        ...
        Intent launch=new Intent(c,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);c.startActivity(launch);Thread.sleep(700);Delivery.start(c);Thread.sleep(300);device.pressHome();waitActivityHidden();Thread.sleep(300);writeDebug(c,device,store,"ready_on_launcher");
        ...
        try{
            for(int i=0;i<CYCLES;i++){
                ...
                tapReaction(device,i%3==0);waitEvents(store,i+1);assertNull("Answered pending must close",store.pending());assertEquals("Stress must not write verified normal progress",0,store.eventCount(true));
            }
