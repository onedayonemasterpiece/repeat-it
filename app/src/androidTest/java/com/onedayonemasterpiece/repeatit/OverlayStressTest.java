...
public final class OverlayStressTest {
    ...
    private static void waitReactionAction(Store store,String expected) throws Exception {long end=System.currentTimeMillis()+UI_TIMEOUT;while(System.currentTimeMillis()<end){if(store.value("last_ui_action","").contains(expected))return;Thread.sleep(20);}fail("Reaction feedback marker did not settle: expected="+expected+" actual="+store.value("last_ui_action",""));}
    ...
    @Test public void nativeOverlaySurvivesFiniteDelayedFortyCycleJourney() throws Exception {
        ...
        for(int i=0;i<CYCLES;i++){Engine.Card card=new Engine.Card();card.deck="stress";card.id="card-"+i;card.title="Stress card "+i;card.text="Synthetic overlay stress payload "+i;cards.add(card);}assertTrue(store.importCards(Map.of("learning/decks/stress.yaml",cards)).isEmpty());store.setMode("agent_debug");store.recompute();assertNull("Agent debug must wait for explicit forceDue instead of spamming autonomously",store.decision().due);
        Intent launch=new Intent(c,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);c.startActivity(launch);Thread.sleep(850);
        UiObject2 testMode=device.wait(Until.findObject(By.textContains("AGENT DEBUG")),UI_TIMEOUT);assertNotNull("Test mode must be obvious in owner UI",testMode);
        UiObject2 sync=device.wait(Until.findObject(By.text("Обновить карточки сейчас")),UI_TIMEOUT);assertNotNull("Primary refresh control must be visible without scrolling",sync);
        UiObject2 pause=device.wait(Until.findObject(By.text("Приостановить тест")),UI_TIMEOUT);assertNotNull("Test pause control must be visible",pause);
        pause.click();UiObject2 resume=device.wait(Until.findObject(By.textContains("Продолжить тест")),UI_TIMEOUT);assertNotNull("Test pause tap must visibly change the control",resume);assertEquals("true",store.value("paused","false"));resume.click();UiObject2 pauseAgain=device.wait(Until.findObject(By.text("Приостановить тест")),UI_TIMEOUT);assertNotNull("Resume tap must visibly restore the test control",pauseAgain);assertEquals("false",store.value("paused","false"));
        ...
        int screenCycles=0,protectedTransitions=0,rotations=0,delayedChecks=0;boolean screenshotChecked=false;
        try{
            for(int i=0;i<CYCLES;i++){
                assertNull("Agent debug cannot self-schedule between explicit steps",store.decision().due);store.forceDue();changed(c);waitPresentation(store);waitControls();waitWindow(device,true);writeDebug(c,device,store,"cycle_"+i+"_presented");Store.Pending presented=store.pending();assertNotNull("Due card must become pending",presented);assertTrue("Pending was not actually presented; delivery_error="+store.value("delivery_error",""),presented.shown>0);assertGeometry(store);assertTrue("Reaction centers must be distinct",OverlayService.rememberX!=OverlayService.repeatX);
                if(!screenshotChecked){File shot=new File(c.getExternalFilesDir(null),"shareable-overlay.png");assertTrue("Overlay should be screenshotable for owner sharing",device.takeScreenshot(shot));assertTrue("Screenshot must contain real pixels",shot.length()>1024);screenshotChecked=true;}
                ...
                boolean repeat=i%3==0;String expectedAction="reaction_"+(repeat?"repeat":"remember")+"_committed";tapReaction(device,repeat);waitEvents(store,i+1);waitReactionAction(store,expectedAction);assertNull("Answered pending must close",store.pending());assertEquals("Stress must not write verified normal progress",0,store.eventCount(true));
            }
