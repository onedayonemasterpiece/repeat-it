...
public final class AdminProvider extends ContentProvider {
    ...
    @Override public Bundle call(String method,String arg,Bundle extras){
        ...
        Instant now=Instant.now();long pendingWait=pending==null?0:Math.max(0,now.toEpochMilli()-pending.created),scheduledLate=d.due==null?0:Math.max(0,Duration.between(d.due,now).toMillis());
        reply.putString("mode",s.mode());reply.putBoolean("paused",s.value("paused","false").equals("true"));reply.putBoolean("overlay_attached",s.value("overlay_attached","false").equals("true"));reply.putBoolean("overlay_visible",s.value("overlay_visible","false").equals("true"));reply.putString("last_ui_action",s.value("last_ui_action",""));reply.putString("pending_id",pending==null?"":pending.id);reply.putLong("pending_created_at",pending==null?0:pending.created);reply.putLong("pending_shown_at",pending==null?0:pending.shown);reply.putLong("pending_wait_ms",pendingWait);reply.putLong("scheduled_due_late_ms",scheduledLate);reply.putInt("cards",s.cards().size());reply.putInt("events",s.eventCount(false));reply.putInt("verified_events",s.eventCount(true));reply.putString("next_due",String.valueOf(d.due));reply.putString("next_card",d.cardKey==null?"":d.cardKey);reply.putString("next_deck",d.deck==null?"":d.deck);reply.putInt("remaining",d.remaining);reply.putInt("overdue",d.expired);reply.putInt("deadline_overdue",d.expired);reply.putString("feasibility",d.feasibility);reply.putString("overlay_geometry",s.value("overlay_geometry",""));
        reply.putString("touch_guard_state",s.value("touch_guard_state","idle"));reply.putString("touch_guard_until_elapsed",s.value("touch_guard_until_elapsed","0"));reply.putBoolean("touch_controls_ready",OverlayService.controlsReady);reply.putInt("touch_guard_blocked",OverlayService.touchGuardBlocked);
        ...
        reply.putString("library_sync_state",s.value("library_sync_state",""));reply.putString("library_sync_error",s.value("library_sync_error",""));reply.putString("last_library_sync",s.value("last_library_sync",""));reply.putString("progress_sync_state",s.value("progress_sync_state",""));reply.putString("progress_sync_error",s.value("progress_sync_error",""));reply.putString("last_progress_sync",s.value("last_progress_sync",""));
