...
public final class MainActivity extends Activity {
    ...
    private Store store;private TextView summary,status;private Button pauseButton,syncButton;
    ...
    @Override public void onCreate(Bundle b){
        ...
        LinearLayout page=new LinearLayout(this);page.setOrientation(LinearLayout.VERTICAL);page.setPadding(dp(18),dp(22),dp(18),dp(22));
        TextView title=new TextView(this);title.setText("Повторение");title.setTextSize(32);page.addView(title);
        TextView subtitle=new TextView(this);subtitle.setText("Карточки появляются сами. Ничего отдельно включать не нужно.");subtitle.setTextSize(18);subtitle.setPadding(0,dp(6),0,dp(14));page.addView(subtitle);
        summary=new TextView(this);summary.setTextSize(20);summary.setPadding(dp(12),dp(12),dp(12),dp(14));page.addView(summary,new LinearLayout.LayoutParams(-1,-2));
        ...
        TextView diagnosticTitle=new TextView(this);diagnosticTitle.setText("Диагностика");diagnosticTitle.setTextSize(18);diagnosticTitle.setPadding(0,dp(20),0,dp(6));page.addView(diagnosticTitle);
        status=new TextView(this);status.setTextSize(14);page.addView(status);
    ...
    private Button button(LinearLayout parent,String label,Runnable action){Button b=new Button(this);b.setText(label);b.setAllCaps(false);b.setTextSize(18);b.setMinHeight(dp(58));b.setOnClickListener(v->action.run());LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,-2);p.setMargins(0,dp(4),0,dp(4));parent.addView(b,p);return b;}
    ...
    private void renderStatus(){
        if(status==null||summary==null)return;boolean paused=store.value("paused","false").equals("true");if(pauseButton!=null)pauseButton.setText(paused?"Продолжить показы":"Поставить на паузу");
        Engine.Decision d=store.decision();Store.Pending p=store.pending();String lastSync=store.value("last_sync","");String syncError=store.value("sync_error","");
        StringBuilder top=new StringBuilder();top.append(paused?"⏸ Показы на паузе":"✓ Показы активны").append("\n");top.append("Карточек: ").append(store.cards().size()).append("\n");top.append(p==null?"Следующий показ: "+nextLabel(d):"Сейчас ждёт ответа одна карточка").append("\n");top.append("Синхронизация: ").append(syncError.isEmpty()?(lastSync.isEmpty()?"ещё не завершалась":"успешно"):"ошибка").append("\n");top.append("Overlay: ").append(Settings.canDrawOverlays(this)?"разрешён":"нужно разрешить");summary.setText(top);
        ...
        s.append("Overlay сейчас виден: ").append(store.value("overlay_visible","false")).append(". Звук следующих карточек: ").append(store.sound()?"включён":"выключен").append("\n");
        ...
        s.append("Последнее действие: ").append(store.value("last_ui_action","ещё нет")).append("\n");s.append("Запрошено обновление: ").append(store.value("sync_requested_at","ещё нет")).append("\n");s.append("Последняя успешная синхронизация: ").append(lastSync.isEmpty()?"ещё нет":lastSync).append("\n");s.append("idea-hub snapshot: ").append(store.value("last_sync_source_sha","ещё нет")).append("\n");
        String delivery=store.value("delivery_error","");if(!syncError.isEmpty())s.append("Sync: ").append(syncError).append("\n");if(!delivery.isEmpty())s.append("Delivery: ").append(delivery).append("\n");s.append("Обновление идёт через WorkManager: после нажатия окно можно закрыть. Force-stop требует ручного открытия. Расход батареи ещё не измерен.");status.setText(s);
    }
