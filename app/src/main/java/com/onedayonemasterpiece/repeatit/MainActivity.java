...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        TextView title=new TextView(this);title.setText("Повторение");title.setTextSize(32);page.addView(title);status=new TextView(this);status.setTextSize(17);page.addView(status);
        ...
        button(page,"Обновить карточки сейчас",()->{SyncWorker.configure(this,true);renderStatus();});
        ...
        button(page,"Настроить GitHub PAT",()->{
            ...
            AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Fine-grained PAT только для idea-hub")
                ...
                .setView(entry).setPositiveButton("Сохранить",(d,w)->{try{new TokenVault(this).save(entry.getText().toString());entry.getText().clear();SyncWorker.configure(this,true);}catch(Exception e){entry.getText().clear();store.put("sync_error","token_save_failed:"+e.getClass().getSimpleName());}renderStatus();})
        ...
        ScrollView scroll=new ScrollView(this);scroll.addView(page);setContentView(scroll);ContextCompat.registerReceiver(this,changed,new IntentFilter(Delivery.CHANGED),ContextCompat.RECEIVER_NOT_EXPORTED);
    }
    ...
    private void renderStatus(){
        Engine.Decision d=store.decision();Store.Pending p=store.pending();StringBuilder s=new StringBuilder("Самооценка, не проверка знаний.\n");
        ...
        s.append("Кнопка обновления запускает WorkManager: окно приложения после нажатия можно закрыть.\n");
    ...
    @Override protected void onResume(){super.onResume();visible=true;SyncWorker.configure(this,true);sendBroadcast(new Intent(Delivery.CHANGED).setPackage(getPackageName()));Delivery.start(this);renderStatus();}
