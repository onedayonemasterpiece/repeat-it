...
public final class Store extends SQLiteOpenHelper {
    ...
    public synchronized String value(String key,String fallback){
        try(Cursor c=getReadableDatabase().rawQuery("SELECT v FROM kv WHERE k=?",new String[]{key})){return c.moveToFirst()?c.getString(0):fallback;}
    }
    ...
    public synchronized Engine.Window window(){return new Engine.Window(LocalTime.parse(value("window_start","07:40")),LocalTime.parse(value("window_end","23:00")),ZoneId.systemDefault());}
    ...
    private synchronized void replan(Instant now){
        ...
        if(!mode.equals("normal")) {
            d=new Engine.Decision(); List<Engine.Card> all=cards(); all.removeIf(c->!c.active);
            if(!all.isEmpty()) {int position=Integer.parseInt(value("test_position_"+mode,"0"));d.cardKey=all.get(Math.floorMod(position,all.size())).key();d.due=window().add(now,mode.equals("user_demo")?300000:1000);}
        } else {
            Map<String,Engine.State> states=new HashMap<>();List<Long> latency=new ArrayList<>();
            for(Engine.Card c:cards()){Engine.State s=state(c,mode);states.put(c.learningKey(),s);}
            ...
            d=Engine.next(cards(),plans(),states,now,window(),latency);
        }
    ...
    public synchronized void settings(Map<String,Object> data){
        if(Contract.positive(data.get("schema_version"),100)!=1)throw new IllegalArgumentException("settings_schema");
        Map<String,Object>w=Contract.object(data.get("allowed_window"));
        if(!"07:40".equals(w.get("start"))||!"23:00".equals(w.get("end"))||!"device_local".equals(w.get("timezone")))throw new IllegalArgumentException("owner_window_mismatch");
    ...
    public synchronized List<String> importCards(Map<String,List<Engine.Card>> documents){
        ...
        documents.forEach((p,cs)->cs.forEach(c->{grouped.computeIfAbsent(c.key(),k->new ArrayList<>()).add(c);paths.put(c.key(),p);}));
        Map<String,Engine.Card> old=new HashMap<>();cards().forEach(c->old.put(c.key(),c));
        SQLiteDatabase db=getWritableDatabase();db.beginTransaction();try{
            ...
            for(Map.Entry<String,List<Engine.Card>> entry:grouped.entrySet()) {
                if(entry.getValue().size()!=1){errors.add("duplicate_identity:"+entry.getKey());continue;}
                Engine.Card c=entry.getValue().get(0), before=old.get(c.key());
    ...
    public synchronized Pending prepare(Instant now){
        Pending p=pending();if(p!=null)return p;
        Engine.Decision d=decision();if(d.due==null||d.cardKey==null||now.isBefore(d.due))return null;
        for(Engine.Card c:cards())if(c.active&&c.key().equals(d.cardKey)){
            p=new Pending();p.id=UUID.randomUUID().toString();p.mode=mode();p.card=c;p.created=now.toEpochMilli();p.soundAllowed=sound();savePending(p);return p;
        }
        replan(now);return null;
    }
    /** Attempt is persisted BEFORE OS sound; losing a signal is preferable to a duplicate. */
    public synchronized boolean markPresentation(String id,boolean overlay,boolean activeUnlocked){
        Pending p=pending();if(p==null||!p.id.equals(id))return false;
        boolean signal=!p.alertDecided&&activeUnlocked&&p.soundAllowed;
        p.alertDecided=true;
        if(overlay&&p.shown==0)p.shown=clock.millis();savePending(p);return signal;
    }
    public synchronized boolean answer(String id,String reaction){
        Pending p=pending();Instant now=now();
        if(p==null||!p.id.equals(id)||p.shown==0||!window().allowed(now))return false;
        if(!Set.of("remember","repeat").contains(reaction))return false;
        SQLiteDatabase db=getWritableDatabase();db.beginTransaction();try{
            ...
            ContentValues row=new ContentValues();row.put("id",id+":response");row.put("mode",p.mode);row.put("body",JSON.toJson(event));db.insertOrThrow("events",null,row);
            db.setTransactionSuccessful();return true;
        }finally{db.endTransaction();}
    }
    public synchronized void setMode(String mode){
        if(!Set.of("normal","agent_debug","user_demo").contains(mode))throw new IllegalArgumentException("invalid_mode");
        SQLiteDatabase db=getWritableDatabase();db.beginTransaction();try{put("mode",mode);db.delete("pending","mode != 'normal'",null);replan(now());db.setTransactionSuccessful();}finally{db.endTransaction();}
    }
    public synchronized void forceDue(){if(!mode().equals("agent_debug"))throw new SecurityException("agent_debug_only");Engine.Decision d=decision();d.due=window().next(now());put("schedule_"+mode(),JSON.toJson(d));}
    ...
    public synchronized List<Batch> batches(){
        SQLiteDatabase db=getWritableDatabase();db.beginTransaction();try{
            for(String mode:List.of("normal","agent_debug","user_demo")) {
                List<String> ids=new ArrayList<>();StringBuilder body=new StringBuilder();
                try(Cursor c=db.rawQuery("SELECT id,body FROM events WHERE batch IS NULL AND mode=? ORDER BY rowid LIMIT 64",new String[]{mode})){while(c.moveToNext()){ids.add(c.getString(0));body.append(c.getString(1)).append('\n');}}
                if(!ids.isEmpty()) {
                    String id=UUID.randomUUID().toString();ContentValues row=new ContentValues();row.put("id",id);row.put("mode",mode);row.put("body",body.toString());db.insertOrThrow("batches",null,row);
                    ContentValues b=new ContentValues();b.put("batch",id);for(String e:ids)db.update("events",b,"id=?",new String[]{e});
                }
            }
