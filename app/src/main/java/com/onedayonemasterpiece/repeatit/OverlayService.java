...
public final class OverlayService extends Service {
    ...
    private void channels(){
        ...
        NotificationChannel silent=new NotificationChannel(SILENT,"Карточки без звука",NotificationManager.IMPORTANCE_LOW);silent.setSound(null,null);silent.enableVibration(false);n.createNotificationChannel(silent);
        NotificationChannel sound=new NotificationChannel(SOUND,"Карточки со звуком",NotificationManager.IMPORTANCE_DEFAULT);sound.enableVibration(false);
        sound.setSound(Uri.parse("android.resource://"+getPackageName()+"/raw/chime"),new AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_NOTIFICATION).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build());n.createNotificationChannel(sound);
    ...
    private void content(Store.Pending p,boolean signal){
        Instant now=Instant.now();if(!store.window().allowed(now)||!active())return;
        if(locked()&&!previewAllowed(p)){cancelContent();return;}
        NotificationManager manager=getSystemService(NotificationManager.class);
        if(!manager.areNotificationsEnabled())return;
        AudioManager audio=getSystemService(AudioManager.class);
        signal=signal&&!locked()&&audio.getRingerMode()==AudioManager.RINGER_MODE_NORMAL&&manager.getCurrentInterruptionFilter()==NotificationManager.INTERRUPTION_FILTER_ALL;
        String old=store.value("notification_tag","");if(!old.equals(p.id))cancelContent();
        Notification.Builder b=new Notification.Builder(this,signal?SOUND:SILENT).setSmallIcon(R.drawable.ic_repeat).setContentTitle(p.card.title).setContentText(p.card.text).setStyle(new Notification.BigTextStyle().bigText(p.card.text)).setContentIntent(open()).setOnlyAlertOnce(true).setVisibility(Notification.VISIBILITY_PRIVATE).setOngoing(true).setTimeoutAfter(Math.max(1,Duration.between(now,store.window().close(now)).toMillis()));
    ...
    private void show(Store.Pending p){
        ...
        hide();boolean dark=(getResources().getConfiguration().uiMode&Configuration.UI_MODE_NIGHT_MASK)==Configuration.UI_MODE_NIGHT_YES;
        int fg=dark?0xfff5f3ef:0xff24272c,bg=dark?0xff24272c:0xfff5f3ef;
        ...
        GradientDrawable backdrop=new GradientDrawable();backdrop.setColor(bg);backdrop.setCornerRadius(dp(24));root.setBackground(backdrop);root.setElevation(dp(10));
        LinearLayout toolbar=new LinearLayout(this);toolbar.setGravity(Gravity.CENTER_VERTICAL);
        TextView label=text("ПОВТОРЕНИЕ",13,fg);toolbar.addView(label,new LinearLayout.LayoutParams(0,-2,1));
        Button bell=new Button(this);bell.setText(store.sound()?"🔔":"🔕");bell.setTextColor(fg);bell.setTextSize(24);bell.setMinWidth(0);bell.setMinimumWidth(0);bell.setPadding(0,0,0,0);
        GradientDrawable circle=new GradientDrawable();circle.setShape(GradientDrawable.OVAL);circle.setColor(dark?0xff3d434c:0xffe2e5e8);bell.setBackground(circle);bell.setContentDescription(store.sound()?"Выключить звук следующих карточек":"Включить звук следующих карточек");
        bell.setOnClickListener(v->{store.put("sound",String.valueOf(!store.sound()));bell.setText(store.sound()?"🔔":"🔕");bell.setContentDescription(store.sound()?"Выключить звук следующих карточек":"Включить звук следующих карточек");});toolbar.addView(bell,new LinearLayout.LayoutParams(dp(48),dp(48)));root.addView(toolbar);
        ...
        if(!p.card.image.isEmpty()) {
            File image=GitHubSync.imageFile(this,p.card.imageHash);
            if(image.exists()){
                BitmapFactory.Options options=new BitmapFactory.Options();options.inJustDecodeBounds=true;BitmapFactory.decodeFile(image.getPath(),options);int sample=1;while(Math.max(options.outWidth,options.outHeight)/sample>1600)sample*=2;options.inSampleSize=sample;options.inJustDecodeBounds=false;
                Bitmap bitmap=BitmapFactory.decodeFile(image.getPath(),options);if(bitmap!=null){ImageView iv=new ImageView(this);iv.setAdjustViewBounds(true);iv.setImageBitmap(bitmap);iv.setContentDescription(p.card.alt);body.addView(iv,new LinearLayout.LayoutParams(-1,-2));}
            }else body.addView(text(p.card.alt+" · изображение пока недоступно офлайн",16,fg));
            body.addView(text(p.card.caption,16,fg));
        }
    ...
    private void tick(){
        ...
        if(!store.value("enabled","false").equals("true")){hide();cancelContent();stopSelf();return;}
        Instant now=Instant.now();boolean allowed=store.window().allowed(now);
        if(!allowed||store.value("paused","false").equals("true")||!active()||MainActivity.visible) {hide();cancelContent();}
        else {
            boolean fresh=store.pending()==null;Store.Pending p=store.prepare(now);
            if(p!=null){
                if(locked())hide();else show(p);
                boolean signal=store.markPresentation(p.id,panel!=null&&!locked(),fresh&&!locked()&&panel!=null);
                content(p,signal);
            }
        }
        Delivery.arm(this);
        Instant next=allowed?store.window().close(now):store.window().next(now);
        handler.postDelayed(boundary,Math.max(1,Duration.between(now,next).toMillis()));
        PreviewWidget.refresh(this);
    }
