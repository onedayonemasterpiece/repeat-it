...
public final class OverlayService extends Service {
    ...
    @Override public void onCreate(){
        ...
        Notification notification=new Notification.Builder(this,SERVICE).setSmallIcon(R.drawable.ic_repeat).setContentTitle("Повторение включено").setContentText("Новые карточки — по заданному окну; уже показанная ждёт ответа").setOngoing(true).setContentIntent(open()).build();
    ...
    private void show(Store.Pending p){
        ...
        scroll.addView(body);root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));LinearLayout reactions=new LinearLayout(this);reactions.setOrientation(LinearLayout.HORIZONTAL);Button remember=new Button(this);remember.setText("Помню");remember.setTextSize(20);remember.setMinHeight(dp(60));remember.setContentDescription("repeat-it-remember");remember.setOnClickListener(v->{remember.setEnabled(false);if(!locked()&&store.answer(p.id,"remember")){hide();cancelContent();tick();}else remember.setEnabled(true);});Button repeat=new Button(this);repeat.setText("Повторить");repeat.setTextSize(20);repeat.setMinHeight(dp(60));repeat.setContentDescription("repeat-it-repeat");repeat.setOnClickListener(v->{repeat.setEnabled(false);if(!locked()&&store.answer(p.id,"repeat")){hide();cancelContent();tick();}else repeat.setEnabled(true);});reactions.addView(remember,new LinearLayout.LayoutParams(0,-2,1));reactions.addView(repeat,new LinearLayout.LayoutParams(0,-2,1));root.addView(reactions);
    ...
    private void tick(){
        handler.removeCallbacks(boundary);if(!store.value("enabled","false").equals("true")){hide();cancelContent();stopSelf();return;}Instant now=Instant.now();boolean normal=store.mode().equals("normal");Engine.Window w=store.window();boolean mayStart=!normal||w.allowed(now);
