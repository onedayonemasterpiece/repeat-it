...
public final class OverlayService extends Service {
    ...
    @Override public void onCreate(){
        ...
        Notification notification=new Notification.Builder(this,SERVICE).setSmallIcon(R.drawable.ic_repeat).setContentTitle("Повторение активно").setContentText("Карточки показываются автоматически; нажмите, чтобы открыть состояние").setOngoing(true).setContentIntent(open()).build();
    ...
    private void react(Store.Pending p,String reaction,Button button){
        button.setEnabled(false);store.put("last_ui_action","reaction_"+reaction+"_tap@"+Instant.now());
        if(!locked()&&store.answer(p.id,reaction)){
            store.put("last_ui_action","reaction_"+reaction+"_committed@"+Instant.now());Toast.makeText(this,"Ответ сохранён",Toast.LENGTH_SHORT).show();hide();cancelContent();tick();
        }else{
            store.put("last_ui_action","reaction_"+reaction+"_rejected@"+Instant.now());button.setEnabled(true);Toast.makeText(this,"Не удалось сохранить ответ — карточка остаётся открытой",Toast.LENGTH_LONG).show();
        }
    }
    private void show(Store.Pending p){
        ...
        LinearLayout toolbar=new LinearLayout(this);toolbar.setGravity(Gravity.CENTER_VERTICAL);TextView label=text("ПОВТОРЕНИЕ",13,fg);toolbar.addView(label,new LinearLayout.LayoutParams(0,-2,1));Button bell=new Button(this);bell.setText(store.sound()?"🔔":"🔕");bell.setTextColor(fg);bell.setTextSize(24);bell.setMinWidth(0);bell.setMinimumWidth(0);bell.setPadding(0,0,0,0);GradientDrawable circle=new GradientDrawable();circle.setShape(GradientDrawable.OVAL);circle.setColor(dark?0xff3d434c:0xffe2e5e8);bell.setBackground(circle);bell.setContentDescription(store.sound()?"Выключить звук следующих карточек":"Включить звук следующих карточек");bell.setOnClickListener(v->{boolean next=!store.sound();store.put("sound",String.valueOf(next));store.put("last_ui_action","sound_"+(next?"on":"off")+"@"+Instant.now());bell.setText(next?"🔔":"🔕");bell.setContentDescription(next?"Выключить звук следующих карточек":"Включить звук следующих карточек");Toast.makeText(this,next?"Звук следующих карточек включён":"Звук следующих карточек выключен",Toast.LENGTH_SHORT).show();});toolbar.addView(bell,new LinearLayout.LayoutParams(dp(48),dp(48)));root.addView(toolbar);
        ...
        if(!p.card.image.isEmpty()){File image=GitHubSync.imageFile(this,p.card.imageHash);if(image.exists()){BitmapFactory.Options options=new BitmapFactory.Options();options.inJustDecodeBounds=true;BitmapFactory.decodeFile(image.getPath(),options);int sample=1;while(Math.max(options.outWidth,options.outHeight)/sample>1600)sample*=2;options.inSampleSize=sample;options.inJustDecodeBounds=false;Bitmap bitmap=BitmapFactory.decodeFile(image.getPath(),options);if(bitmap!=null){ImageView iv=new ImageView(this);iv.setAdjustViewBounds(true);iv.setImageBitmap(bitmap);iv.setContentDescription(p.card.alt);body.addView(iv,new LinearLayout.LayoutParams(-1,-2));}}else body.addView(text(p.card.alt+" · изображение пока недоступно офлайн",16,fg));body.addView(text(p.card.caption,16,fg));}
        scroll.addView(body);root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));LinearLayout reactions=new LinearLayout(this);reactions.setOrientation(LinearLayout.HORIZONTAL);reactions.setPadding(0,dp(8),0,0);Button remember=new Button(this);remember.setText("Помню");remember.setAllCaps(false);remember.setTextSize(20);remember.setMinHeight(dp(68));remember.setContentDescription("repeat-it-remember");remember.setOnClickListener(v->react(p,"remember",remember));Button repeat=new Button(this);repeat.setText("Повторить");repeat.setAllCaps(false);repeat.setTextSize(20);repeat.setMinHeight(dp(68));repeat.setContentDescription("repeat-it-repeat");repeat.setOnClickListener(v->react(p,"repeat",repeat));reactions.addView(remember,new LinearLayout.LayoutParams(0,-2,1));reactions.addView(repeat,new LinearLayout.LayoutParams(0,-2,1));root.addView(reactions);
    ...
    private void tick(){
        ...
        if(!active()||MainActivity.visible){hide();cancelContent();}
            ...
            if(p==null){hide();cancelContent();}
                ...
                // A card that was never actually presented before cutoff is not allowed to appear for the first time at night.
