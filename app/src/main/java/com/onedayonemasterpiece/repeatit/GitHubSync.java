...
public final class GitHubSync {
    ...
    private Response request(String path,String method,boolean raw,String etag,String body,int limit) throws IOException {
        Contract.readPath(path);if(!method.equals("GET"))Contract.writePath(store.device(),path);String suffix="";
        if(method.equals("GET"))suffix="?ref="+URLEncoder.encode(readRef(path),"UTF-8");URL url=new URL(ROOT+path+suffix);HttpsURLConnection conn=(HttpsURLConnection)url.openConnection();
        conn.setInstanceFollowRedirects(false);conn.setConnectTimeout(15000);conn.setReadTimeout(20000);conn.setRequestMethod(method);conn.setRequestProperty("Authorization","Bearer "+token);conn.setRequestProperty("Accept",raw?"application/vnd.github.raw+json":"application/vnd.github+json");conn.setRequestProperty("X-GitHub-Api-Version","2022-11-28");conn.setRequestProperty("User-Agent","Repeat-It/1");
        ...
        try{Response result=new Response();result.status=conn.getResponseCode();result.etag=conn.getHeaderField("ETag");if(result.status==304){result.bytes=new byte[0];result.body="";return result;}if(result.status<200||result.status>=300)throw new RemoteError(result.status);
            try(InputStream in=conn.getInputStream();ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] buf=new byte[8192];int n,total=0;while((n=in.read(buf))!=-1){total+=n;if(total>limit)throw new IOException("document_size_limit");out.write(buf,0,n);}result.bytes=out.toByteArray();}
    ...
    private Map<String,Object> parse(String body){
        if(body.startsWith("\uFEFF"))body=body.substring(1);String stripped=body.trim();if((stripped.startsWith("```json\n")||stripped.startsWith("```yaml\n"))&&stripped.endsWith("```")){body=stripped.substring(stripped.indexOf('\n')+1,stripped.length()-3);issues.add("removed_document_fence");}
        LoaderOptions options=new LoaderOptions();options.setAllowDuplicateKeys(false);options.setMaxAliasesForCollections(0);options.setNestingDepthLimit(40);options.setCodePointLimit(Contract.MAX_DOCUMENT_BYTES);return Contract.object(new Yaml(new SafeConstructor(options)).load(body));
    }
    ...
    private void loadKnowledge(String ref) throws IOException {
        sourceRef=ref;Map<String,List<Engine.Card>> material=new LinkedHashMap<>();Set<String> remotePaths=discover(material);issues.addAll(store.importCards(material,remotePaths));
        for(String path:List.of("learning/settings.json","learning/plans.json")){try{Response r=read(path,true);Map<String,Object> data=parse(r.body);if(path.endsWith("settings.json"))store.settings(data);else store.setPlans(Contract.plans(data));store.cache(path,r.etag,r.body);}catch(Exception e){problem(path,e);}}
